"""
XRP/USDT Telegram Signal Bot
-----------------------------
Price source: Coinbase public spot price endpoint (XRP-USD, no API key required)
...
@app.route("/")
def home():
    return "XRP/USDT bot is alive.", 200
...
# ---------------------------------------------------------------------------
def fetch_price():
    """Fetch the latest XRP/USD spot price from Coinbase. Returns float or None.
...
def fetch_candles(granularity=STRUCTURE_GRANULARITY_SECONDS, limit=STRUCTURE_LOOKBACK):
    ...
    try:
        resp = requests.get(
            ...
            headers={"User-Agent": "xrp-signal-bot/1.0"},
            timeout=15,
        )
        resp.raise_for_status()
        raw = resp.json()
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if price is not None:
        ...
        await update.message.reply_text(
            "⚠️ Couldn't fetch the current XRP price. Try again shortly."
        )
...
async def price_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    price = fetch_price()
    if price is not None:
        await update.message.reply_text(f"💧 XRP/USDT: ${price:,.2f}")
    else:
        await update.message.reply_text("⚠️ Couldn't fetch the current XRP price.")
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "refresh":
        ...
        if price is not None:
            ...
            await query.edit_message_text("⚠️ Couldn't fetch the current XRP price.")
...
def main():
    ...
    logger.info("XRP/USDT bot starting...")
