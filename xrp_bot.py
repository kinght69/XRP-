...
communication. All trading intelligence lives in backtest_xrp.py — this
...
bot_logger = logging.getLogger("xrp_bot")
error_logger = logging.getLogger("xrp_bot.errors")
...
@flask_app.route("/")
def home():
    return "XRP signal bot is alive.", 200
...
def format_analysis_message(result):
    ...
    lines = [f"{emoji} *XRP/USDT — {signal}*", f"💰 Price: ${result['price']:,.2f}"]
...
async def price_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if price is not None:
        ...
        await update.message.reply_text("⚠️ Couldn't fetch the current XRP price. Try again shortly.")
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "start":
        ...
        price = with_retries(engine.fetch_spot_price, max_retries=2)
        text = f"✕ XRP/USDT: ${price:,.2f}" if price is not None else "⚠️ Couldn't fetch price."
...
def main():
    ...
    bot_logger.info("XRP/USDT bot starting...")
